                  ...
                  {teamMembers.length === 0 ? (
                    ...
                    return (
                      <tr key={m.id} className={`border-b border-default/50 transition-colors ${!m.is_active ? 'opacity-50' : 'hover:bg-white/[0.02]'}`}>
                        ...
                        <td className="py-3 px-4">
                          <span style={{ background: rc.bg, color: rc.color, border: `1px solid ${rc.border}`, fontSize: '10px', fontWeight: '700', padding: '3px 10px', borderRadius: '20px', whiteSpace: 'nowrap' }}>
                            {{ admin:'ADMIN', manager:'MANAGER', achizitii:'ACHIZIȚII', marketing:'MARKETING', suport:'SUPORT', garantii_service:'GARANTII & SERVICE' }[m.role] || m.role.toUpperCase()}
                          </span>
                        </td>
